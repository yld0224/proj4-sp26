                                           ...
                                           ResultWriter& writer)
{
  ...
  if (!stmt.primary_key_.empty())
  {
    ...
    index = catalog_->CreateIndex<IntegerKeyType, IntegerValueType,
                                  IntegerComparatorType>(
        txn, stmt.table_ + "_pk", stmt.table_, info->schema_, key_schema,
        col_ids, TWO_INTEGER_SIZE, IntegerHashFunctionType{}, true);
  }
  l.unlock();
  ...
  if (index != nullptr)
  {
    WriteOneCell(fmt::format("Table created with id = {}, Primary key index "
                             "created with id = {}",
                             info->oid_, index->index_oid_),
                 writer);
  }
  else
  {
                                          ...
                                          ResultWriter& writer)
{
  ...
  else if (stmt.index_type_ == "hash")
  {
    info = catalog_->CreateIndex<IntegerKeyType, IntegerValueType,
                                 IntegerComparatorType>(
        txn, stmt.index_name_, stmt.table_->table_, stmt.table_->schema_,
        key_schema, col_ids, TWO_INTEGER_SIZE, IntegerHashFunctionType{}, false,
  ...
  WriteOneCell(fmt::format("Index created with id = {} with type = {}",
                           info->index_oid_, info->index_type_),
               writer);
}
...
void BustubInstance::HandleTxnStatement(Transaction* txn,
                                        const TransactionStatement& stmt,
                                        ResultWriter& writer)
{
  ...
  {
    writer.OneCell(fmt::format(
        "{}txn_id={} txn_real_id={} read_ts={} commit_ts={} status={} "
        "iso_lvl={}",
        prefix, current_txn_->GetTransactionIdHumanReadable(),
        current_txn_->GetTransactionId(), current_txn_->GetReadTs(),
        current_txn_->GetCommitTs(), current_txn_->GetTransactionState(),
        current_txn_->GetIsolationLevel()));
  };
  if (txn == nullptr)
  {
  ...
  if (stmt.type_ == "begin")
  {
    ...
    {
      writer.OneCell(
          "begin statement is only supported in managed txn mode, please use "
          "bustub-shell");
      return;
    }
    bool txn_activated = current_txn_ != nullptr;
    auto iso_lvl =
        StringUtil::Lower(GetSessionVariable("global_isolation_level"));
    ...
    dump_current_txn(txn_activated ? "pause current txn and begin new txn "
                                   : "begin txn ");
    return;
  }
  if (stmt.type_ == "commit")
  {
    ...
    if (res)
    {
      writer.OneCell(fmt::format(
          ...
          txn->GetReadTs(), txn->GetCommitTs()));
    }
    else
    {
      writer.OneCell(fmt::format(
          "txn failed to commit, txn_id={}, status={}, read_ts={}, "
          "commit_ts={}",
          txn->GetTransactionIdHumanReadable(), txn->GetTransactionState(),
          txn->GetReadTs(), txn->GetCommitTs()));
    }
  ...
  if (stmt.type_ == "abort")
  {
    ...
    writer.OneCell(fmt::format("txn aborted, txn_id={}, status={}, read_ts={}",
