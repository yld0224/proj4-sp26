                               ...
                               AbstractPlanNodeRef child) -> AbstractPlanNodeRef
{
  ...
  for (uint32_t i = 0; i < statement.select_list_.size(); i++)
  {
    ...
    columns.emplace_back(std::make_shared<ColumnValueExpression>(
        0, -1, Column{"<placeholder>", TypeId::INTEGER}));
    ...
    {
      throw Exception(
          "Bustub currently only support window function with default window "
          "frame settings");
    }
