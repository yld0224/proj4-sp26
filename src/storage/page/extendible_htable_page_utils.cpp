...
void ExtendibleHTableDirectoryPage::PrintDirectory() const
{
  ...
  for (uint32_t idx = 0; idx < static_cast<uint32_t>(0x1 << global_depth_);
       idx++)
  {
    LOG_DEBUG("|    %u    |    %u    |    %u    |", idx, bucket_page_ids_[idx],
              local_depths_[idx]);
  }
...
void ExtendibleHTableDirectoryPage::VerifyIntegrity() const
{
  ...
  for (uint32_t curr_idx = 0; curr_idx < Size(); curr_idx++)
  {
    ...
    // Verify: (1) All LD <= GD.
    BUSTUB_ASSERT(curr_ld <= global_depth_,
        ...
        curr_ld != page_id_to_ld[curr_page_id])
    {
      ...
      LOG_WARN(
          "Verify Integrity: curr_local_depth: %u, old_local_depth %u, for "
          "page_id: %u",
  ...
  while (it != page_id_to_count.end())
  {
    ...
    {
      LOG_WARN(
          "Verify Integrity: curr_count: %u, required_count %u, for page_id: "
          "%u",
          curr_count, required_count, curr_page_id);
      PrintDirectory();
      BUSTUB_ASSERT(
          curr_count == required_count,
          "a bucket does not have precisely 2^(GD - LD) pointers to it");
    }
    it++;
  }
}
