...
{
  Context ctx;
  ctx.write_set_.push_back(bpm_ -> FetchPageWrite(header_page_id_));//直接让headerpage入栈
  WritePageGuard& headerPageGuard = ctx.write_set_.front();
  if (headerPageGuard.template AsMut<BPlusTreeHeaderPage>() -> root_page_id_ == INVALID_PAGE_ID){
    page_id_t id;
  ...
  ctx.root_page_id_ = headerPageGuard.template AsMut<BPlusTreeHeaderPage>() -> root_page_id_;
  auto currentPageGuard = bpm_ -> FetchPageWrite(ctx.root_page_id_);
  while (!currentPageGuard.template As<BPlusTreePage>() -> IsLeafPage()) {
    const InternalPage* currentInternalPage = currentPageGuard.template As<InternalPage>();
    int index = BinaryFind(currentInternalPage, key);
    if (currentInternalPage -> GetSize() < currentInternalPage -> GetMaxSize()){
      ctx.write_set_.clear();
    }//悲观螃蟹法则,当前节点如果安全,释放祖先节点的所有写锁
    ctx.write_set_.push_back(std::move(currentPageGuard));
  ...
  if (targetLeaf -> GetMaxSize() < targetLeaf -> GetSize()) {
    ...
    KeyType upKey = targetLeaf -> KeyAt(index);
    page_id_t newPageId;
    ...
    if (currentPageGuard.PageId() == ctx.root_page_id_) {
      page_id_t newRootId;
      ...
      newRoot -> SetValueAt(1, newPageId);
      newRoot -> SetValueAt(0, currentPageGuard.PageId());
      return true;
    }//情况1: 叶子节点并且是根节点,新建根节点即可直接返回
    WritePageGuard& parentGuard = ctx.write_set_.back();
    InternalPage* parentPage = parentGuard.template AsMut<InternalPage>();
    int keyPosition = BinaryFind(parentPage, upKey);//重新查找一遍,因数据可能被其他线程修改 fix:先查找再increase
    parentPage -> IncreaseSize(1);
    for (int i = parentPage -> GetSize() - 1; i > keyPosition + 1 ; --i) {
      parentPage -> SetKeyAt(i, parentPage -> KeyAt(i - 1));
      parentPage -> SetValueAt(i, parentPage -> ValueAt(i - 1));
    }
    parentPage -> SetKeyAt(keyPosition + 1, upKey);
    parentPage -> SetValueAt(keyPosition + 1, newPageId);
    InternalPage* currentPage = parentPage;
    page_id_t currentId = parentGuard.PageId();
    currentPageGuard.Drop();
    ...
    while (currentPage -> GetMaxSize() < currentPage -> GetSize()) {
      ...
      if (currentId != ctx.root_page_id_) {
        WritePageGuard& parentGuard = ctx.write_set_[ctx.write_set_.size() - 2];
        parentPage = parentGuard.template AsMut<InternalPage>();
        keyPosition = BinaryFind(parentPage, midKey);//fix:先查找
        parentPage -> IncreaseSize(1);
        for (int i = parentPage -> GetSize() - 1; i > keyPosition + 1 ; --i) {
          parentPage -> SetKeyAt(i, parentPage -> KeyAt(i - 1));
          parentPage -> SetValueAt(i, parentPage -> ValueAt(i - 1));
        }
        parentPage -> SetKeyAt(keyPosition + 1, midKey);
        parentPage -> SetValueAt(keyPosition + 1, rightPageId);
        currentPage = parentPage;
        currentId = parentGuard.PageId();
        ctx.write_set_.pop_back(); //fix:最后释放锁
        continue;
      }//情况3: 内部节点并且不是根节点
      page_id_t newRootId;
      auto newRootGuard = bpm_ -> NewPageGuarded(&newRootId);
      ...
      newRoot -> SetValueAt(0, currentId);
      return true;
      //情况4: 内部节点并且是根节点
    }
  }//插入之后的维护
...
{
  Context ctx;
  ctx.write_set_.push_back(bpm_ -> FetchPageWrite(header_page_id_));
  WritePageGuard& headerPageGuard = ctx.write_set_.front();
  if (headerPageGuard.template AsMut<BPlusTreeHeaderPage>() -> root_page_id_ == INVALID_PAGE_ID) {
    return;
  }
  ...
  while (!currentPageGuard.template As<BPlusTreePage>() -> IsLeafPage()) {
    const InternalPage* currentInternalPage = currentPageGuard.template As<InternalPage>();
    int index = BinaryFind(currentInternalPage, key);
    if (currentInternalPage -> GetSize() > currentInternalPage -> GetMinSize()) {
      ctx.write_set_.clear();
    }
    ctx.write_set_.push_back(std::move(currentPageGuard));
    currentPageGuard = bpm_ -> FetchPageWrite(currentInternalPage -> ValueAt(index));
  ...
  if (currentPageGuard.PageId() == ctx.root_page_id_) {
    if (targetLeaf -> GetSize() == 0) {
      ...
      page_id_t vicPage = currentPageGuard.PageId();
      currentPageGuard.Drop();//fix: 先drop之后才delete
      bpm_ -> DeletePage(vicPage);
    }
    return;
  }//当前节点已经是根节点
  WritePageGuard& targetParentGuard = ctx.write_set_.back();
  InternalPage* fatherPage = targetParentGuard.template AsMut<InternalPage>();
  int pos = BinaryFind(fatherPage, key);
  if (pos > 0) {
    auto leftPageGuard = bpm_ -> FetchPageWrite(fatherPage ->ValueAt(pos - 1));
    ...
    if (leftPage -> GetSize() > leftPage -> GetMinSize()) {
      ...
      fatherPage -> SetKeyAt(pos, targetLeaf -> KeyAt(0));
      return;
    }
  }
  if (pos <= fatherPage -> GetSize() - 2) {
    auto rightPageGuard = bpm_ -> FetchPageWrite(fatherPage -> ValueAt(pos + 1));
    ...
    if (rightPage -> GetSize() > rightPage -> GetMinSize()) {
      ...
      fatherPage -> SetKeyAt(pos + 1, rightPage -> KeyAt(0));
      return;
    }
  }//尝试从左右兄弟redistribute
  if (pos > 0) {
    ...
    for (int i = pos; i < fatherPage -> GetSize() - 1; ++i){
      fatherPage -> SetKeyAt(i, fatherPage -> KeyAt(i + 1));
      fatherPage -> SetValueAt(i, fatherPage -> ValueAt(i + 1));
    }
    fatherPage -> IncreaseSize(-1);
    page_id_t vicPage = fatherPage -> ValueAt(pos);
    currentPageGuard.Drop(); 
    bpm_ -> DeletePage(vicPage);
  } else {
    auto rightPageGuard = bpm_ -> FetchPageWrite(fatherPage -> ValueAt(pos + 1));
    ...
    for (int i = pos + 1; i < fatherPage -> GetSize() - 1; ++i) {
      fatherPage -> SetKeyAt(i, fatherPage -> KeyAt(i + 1));
      fatherPage -> SetValueAt(i, fatherPage -> ValueAt(i + 1));
    }
    fatherPage -> IncreaseSize(-1);
    page_id_t vicPage = fatherPage -> ValueAt(pos + 1);
    rightPageGuard.Drop(); 
    bpm_ -> DeletePage(vicPage);
    currentPageGuard.Drop();
  }//从左右兄弟合并,转内部节点的调整
  InternalPage* currentPage = fatherPage;
  while (currentPage -> GetSize() < currentPage -> GetMinSize()) {
    WritePageGuard& currentGuard = ctx.write_set_.back();
    if (ctx.write_set_.front().PageId() == header_page_id_ && ctx.write_set_.size() == 2) {
      if (currentPage -> GetSize() == 1) {
        ...
        page_id_t vicPage = currentGuard.PageId();
        ctx.write_set_.pop_back();
        bpm_ -> DeletePage(vicPage);
      }
      return;
    }//如果已经上升到根节点依然underflow,若根只有单独节点则换根,否则返回
    WritePageGuard& currentParentGuard = ctx.write_set_[ctx.write_set_.size() - 2];
    InternalPage* nextPage = currentParentGuard.template AsMut<InternalPage>();
    pos = BinaryFind(nextPage, key);
    if (pos > 0) {
    ...
    if (pos > 0) {
      ...
      page_id_t vicPage = nextPage -> ValueAt(pos);
      ctx.write_set_.pop_back();
      bpm_ -> DeletePage(vicPage);
      ...
      nextPage -> IncreaseSize(-1);
      page_id_t vicPage = nextPage -> ValueAt(pos + 1);
      rightPageGuard.Drop(); 
      bpm_ -> DeletePage(vicPage);
      currentPage = nextPage;
      ctx.write_set_.pop_back();
