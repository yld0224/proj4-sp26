...
static const char* ta_list_2023_fall[] = {
    "skyzh",     "yliang412",       "fernandolis10", "wiam8",
...
static const char* ta_oh_2023_fall[] = {
    "Randomly", "Tuesday",   "Wednesday", "Tuesday", "Thursday", "Tuesday",
...
auto GetFunctionOf(const MockScanPlanNode* plan) -> std::function<Tuple(size_t)>
{
  ...
  if (table == "__mock_table_tas_2023_fall")
  {
    return [plan](size_t cursor)
    {
      std::vector<Value> values{};
      values.push_back(
          ValueFactory::GetVarcharValue(ta_list_2023_fall[cursor]));
  ...
  if (table == "__mock_t9")
  {
    return [plan](size_t cursor)
    {
      std::vector<Value> values{};
      values.push_back(ValueFactory::GetIntegerValue(cursor / 10000));
      values.push_back(ValueFactory::GetIntegerValue(
          10000000 -
          (cursor / 2 + ((cursor / 10000) % 2) * ((cursor / 2) % 2))));
      return Tuple{values, &plan->OutputSchema()};
    };
  }
