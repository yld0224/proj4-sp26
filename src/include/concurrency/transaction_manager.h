...
/// The first undo link in the version chain, that links table heap tuple to the
/// undo log.
struct VersionUndoLink
{
  /** The next version in the version chain. */
  UndoLink prev_;
  /** Whether a transaction is modifying the version link. Fall 2023: you do not
   * need to read / write this field until task 4.2. */
  ...
  inline static auto FromOptionalUndoLink(std::optional<UndoLink> undo_link)
...
class TransactionManager
{
   ...
   */
  auto Begin(IsolationLevel isolation_level =
                 IsolationLevel::SNAPSHOT_ISOLATION) -> Transaction*;
  ...
  /**
   * @brief Use this function before task 4.2. Update an undo link that links
   * table heap tuple to the first undo log. Before updating, `check` function
   * will be called to ensure validity.
   */
  auto UpdateUndoLink(
  ...
  /**
   * @brief Use this function after task 4.2. Update an undo link that links
   * table heap tuple to the first undo log. Before updating, `check` function
   * will be called to ensure validity.
   */
  auto UpdateVersionLink(
      RID rid, std::optional<VersionUndoLink> prev_version,
      std::function<bool(std::optional<VersionUndoLink>)>&& check = nullptr)
      -> bool;

  /** @brief Get the first undo log of a table heap tuple. Use this before
   * task 4.2 */
  auto GetUndoLink(RID rid) -> std::optional<UndoLink>;

  /** @brief Get the first undo log of a table heap tuple. Use this after
   * task 4.2 */
   ...
   * nullopt if the txn does not exist. Will still throw an exception if the
