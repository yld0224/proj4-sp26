...
class TableHeap
{
   ...
   * together, use `GetTuple` instead to ensure atomicity.
   * @param rid rid of the tuple to read
   * @return the meta
   */
  auto GetTupleMeta(RID rid) -> TupleMeta;

  /** @return the iterator of this table. When this iterator is created, it will
   * record the current last tuple in the table heap, and the iterator will stop
   * at that point, in order to avoid halloween problem. You usually will need
   * to use this function for project 3. Given that you have already implemented
   * your project 4 update executor as a pipeline breaker, you may use
   * `MakeEagerIterator` to test whether the update executor is implemented
   * correctly. There should be no difference between this function and
   * `MakeEagerIterator` in project 4 if everything is implemented correctly. */
  auto MakeIterator() -> TableIterator;

  /** @return the iterator of this table. The iterator will stop at the last
   * tuple at the time of iterating. */
  ...
  /**
   * Update a tuple in place. Should NOT be used in project 3. Implement your
   * project 3 update executor as delete and insert. You will need to use this
   * function in project 4.
   * @param meta new tuple meta
  ...
  /** For binder tests */
  static auto CreateEmptyHeap(bool create_table_heap = false)
      -> std::unique_ptr<TableHeap>
  {
  ...
  // The below functions are useful only when you want to implement abort in a
  // way that removes an undo log from the version chain. DO NOT USE THEM if you
  // are unsure what they are supposed to do.
  //
  // And if you decide to use the below functions, DO NOT use the normal ones
  // like `GetTuple`. Having two read locks on the same thing in one thread
  ...
  void UpdateTupleInPlaceWithLockAcquired(const TupleMeta& meta,
                                          const Tuple& tuple, RID rid,
                                          TablePage* page);

  auto GetTupleWithLockAcquired(RID rid, const TablePage* page)
      -> std::pair<TupleMeta, Tuple>;

  auto GetTupleMetaWithLockAcquired(RID rid, const TablePage* page)
      -> TupleMeta;
