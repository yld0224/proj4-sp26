...
/**
 * WriteBackCache provides extra memory space other than the buffer pool to
 * store the pages. It's purpose is to gather the copy of pages that are about
 * to be written back to disk, so that the bpm doesn't have to incur IO penality
 * and wait for the write to be completed when evicting. Spring 24: The cache is
 * limited to store a constant number of pages in total (8).
 * !! ANY ATTEMPTS TO ADD ANOTHER IN-MEMORY CACHE WILL BE REVIEWED MANUALLY AS
 * PER LEADERBOARD POLICY !!
 */
class WriteBackCache
{
   ...
   * @return pointer to the copied page in the cache, or nullptr if the cache is
   * full.
   */
  auto Add(Page* page) -> Page*
  {
    ...
    memcpy(write_back_pages_[slot].GetData(), page->GetData(),
           BUSTUB_PAGE_SIZE);
    MarkSlotUsed(slot);
  ...
  {
    BUSTUB_ASSERT(((free_slot_bitmap_ >> slot) & 1U) == 0,
  ...
  {
    BUSTUB_ASSERT(((free_slot_bitmap_ >> slot) & 1U) == 1,
