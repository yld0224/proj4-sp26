...
class PlannerContext
{
   ...
   * the vector should be used over the output from the aggregation plan node.
   */
...
class Planner
{
  ...
  auto PlanSelectWindow(const SelectStatement& statement,
                        AbstractPlanNodeRef child) -> AbstractPlanNodeRef;
  ...
  auto GetWindowAggCallFromFactory(const std::string& func_name,
                                   std::vector<AbstractExpressionRef> args)
