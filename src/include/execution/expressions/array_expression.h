...
class ArrayExpression : public AbstractExpression
{
      ...
      -> Value override
  {
    ...
    for (const auto& expr : children_)
    {
      ...
      {
        throw Exception(
            "vector value can only be constructed from decimal type");
      }
      ...
      -> Value override
  {
    ...
    for (const auto& expr : children_)
    {
      auto val = expr->EvaluateJoin(left_tuple, left_schema, right_tuple,
                                    right_schema);
      if (val.GetTypeId() != TypeId::DECIMAL)
      {
        throw Exception(
  ...
  {
    return fmt::format("[{}]", fmt::join(children_, ","));
  }
