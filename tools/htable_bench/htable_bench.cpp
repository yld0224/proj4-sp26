...
auto main(int argc, char** argv) -> int
{
  ...
  program.add_argument("--duration")
      .help("run htable bench for n milliseconds");
  ...
  auto bpm = std::make_unique<BufferPoolManager>(
      BUSTUB_BPM_SIZE, disk_manager.get(), LRU_K_SIZE);

  fmt::print(
      stderr,
      "[info] total_keys={}, duration_ms={}, lru_k_size={}, bpm_size={}\n",
      TOTAL_KEYS, duration_ms, LRU_K_SIZE, BUSTUB_BPM_SIZE);

  auto key_schema = bustub::ParseCreateStatement("a bigint");
  bustub::GenericComparator<8> comparator(key_schema.get());

  bustub::DiskExtendibleHashTable<bustub::GenericKey<8>, bustub::RID,
                                  bustub::GenericComparator<8>>
      index("foo_pk", bpm.get(), comparator,
            bustub::HashFunction<bustub::GenericKey<8>>(), HTABLE_HEADER_DEPTH,
  ...
  for (size_t thread_id = 0; thread_id < BUSTUB_READ_THREAD; thread_id++)
  {
    threads.emplace_back(std::thread(
        [thread_id, &index, duration_ms, &total_metrics]
        {
          HTableMetrics metrics(fmt::format("read  {:>2}", thread_id),
                                duration_ms);
          metrics.Begin();
          ...
          while (!metrics.ShouldFinish())
          {
            ...
            size_t cnt = 0;
            for (auto key = base_key; key < key_end && cnt < KEY_MODIFY_RANGE;
                 key++, cnt++)
            {
              rids.clear();
              index_key.SetFromInteger(key);
              ...
              if (!KeyWillVanish(key) && !KeyWillNeverBeInserted(key) &&
                  rids.empty())
              {
                  ...
                  !KeyWillChange(key))
              {
                ...
                if (static_cast<size_t>(rids[0].GetPageId()) != key ||
                    static_cast<size_t>(rids[0].GetSlotNum()) != key)
                {
                  std::string msg =
                      fmt::format("invalid data: {} -> {}", key, rids[0].Get());
                  throw std::runtime_error(msg);
                }
              }
              metrics.Tick();
  ...
  for (size_t thread_id = 0; thread_id < BUSTUB_WRITE_THREAD; thread_id++)
  {
    threads.emplace_back(std::thread(
        [thread_id, &index, duration_ms, &total_metrics]
        {
          HTableMetrics metrics(fmt::format("write {:>2}", thread_id),
                                duration_ms);
          metrics.Begin();
