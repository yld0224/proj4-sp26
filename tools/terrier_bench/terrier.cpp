...
struct TerrierTotalMetrics
{
  ...
  {
    auto transfer_txn_per_sec =
        committed_transfer_txn_cnt_ / static_cast<double>(elapsed_) * 1000;
    auto join_txn_per_sec =
        committed_join_txn_cnt_ / static_cast<double>(elapsed_) * 1000;
...
struct TerrierMetrics
{
  ...
  void Report()
  {
    ...
    {
      fmt::print(stderr,
                 "[{:5.2f}] {}: total_committed_txn={:<8} "
                 "total_aborted_txn={:<8} throughput={:<8.3f} "
                 "avg_throughput={:<8.3f}\n",
                 elapsed / 1000.0, reporter_, committed_txn_cnt_,
                 aborted_txn_cnt_,
                 (committed_txn_cnt_ - last_committed_txn_cnt_) /
                     static_cast<double>(elapsed - last_report_at_) * 1000,
...
void Bench1TaskTransfer(const int thread_id, const int terrier_num,
                        const uint64_t duration_ms,
                        const bustub::IsolationLevel iso_lvl,
                        bustub::BustubInstance* bustub,
                        TerrierTotalMetrics& total_metrics)
{
  ...
  std::uniform_int_distribution<int> money_transfer_dist(5,
                                                         max_transfer_amount);
  ...
  while (!metrics.ShouldFinish())
  {
    ...
    std::string query1 =
        fmt::format("UPDATE terriers SET token = token + {} WHERE terrier = {}",
                    transfer_amount, terrier_a);
    std::string query2 =
        fmt::format("UPDATE terriers SET token = token - {} WHERE terrier = {}",
                    transfer_amount, terrier_b);
  ...
  total_metrics.ReportTransfer(metrics.aborted_txn_cnt_,
                               metrics.committed_txn_cnt_);
}

void Bench2TaskTransfer(const int thread_id, const int terrier_num,
                        const uint64_t duration_ms,
                        const bustub::IsolationLevel iso_lvl,
                        bustub::BustubInstance* bustub,
                        TerrierTotalMetrics& total_metrics,
                        std::atomic<int>& token_adjustment)
{
  const int max_transfer_amount = 1000;
  std::random_device r;
  std::default_random_engine gen(r());
  ...
  while (!metrics.ShouldFinish())
  {
    ...
    int transfer_amount = money_transfer_dist(gen);
    auto txn = bustub->txn_manager_->Begin(iso_lvl);
    std::string select1 = fmt::format(
        "SELECT network FROM terriers WHERE terrier = {}", terrier_a);
    std::string select2 = fmt::format(
        "SELECT network FROM terriers WHERE terrier = {}", terrier_b);
    ...
    int receive =
        network1 == network2 ? transfer_amount : transfer_amount * 0.97;
...
void Bench2TaskJoin(const int thread_id, const int terrier_num,
                    const uint64_t duration_ms,
                    const bustub::IsolationLevel iso_lvl,
                    bustub::BustubInstance* bustub,
                    TerrierTotalMetrics& total_metrics,
                    std::atomic<int>& token_adjustment)
{
  ...
  while (!metrics.ShouldFinish())
  {
    ...
    if (join_action == 0)
    {
      ...
      auto txn = bustub->txn_manager_->Begin(iso_lvl);
      std::string target_network_sql = fmt::format(
          "SELECT network FROM terriers WHERE terrier = {}", join_target);
      ...
      std::string join_sql = fmt::format(
          "UPDATE terriers SET network = {}, token = token + {} WHERE terrier "
          "= {}",
    ...
    if (join_action == 2)
    {
      // trigger serializable verification by two txns join each other, one
      // should abort
      ...
      std::string network1_sql = fmt::format(
          "SELECT network FROM terriers WHERE terrier = {}", terrier2);
      std::string network2_sql = fmt::format(
          "SELECT network FROM terriers WHERE terrier = {}", terrier1);
      ...
      std::string join1_sql = fmt::format(
          "UPDATE terriers SET network = {}, token = token + {} WHERE terrier "
          "= {}",
          network2, sign_bonus, terrier1);
      std::string join2_sql = fmt::format(
          "UPDATE terriers SET network = {}, token = token + {} WHERE terrier "
          "= {}",
  ...
  token_adjustment.fetch_add(adjustment);
  total_metrics.ReportJoin(metrics.aborted_txn_cnt_,
                           metrics.committed_txn_cnt_);
}

auto ComputeDbSize(bustub::BustubInstance* bustub,
...
void TaskComputeDbSize(const uint64_t duration_ms, std::atomic<int>& db_size,
                       bustub::BustubInstance* bustub,
                       const std::string& table_name)
{
...
void PrintPlan(bustub::BustubInstance& instance, const std::string& query,
               bool ensure_index_scan = true)
{
  {
    std::stringstream ss;
...
auto main(int argc, char** argv) -> int
{
  ...
  program.add_argument("--duration")
      .help("run terrier bench for n milliseconds");
  ...
  program.add_argument("--commit-threshold")
      .help("number of commits to pass the benchmark");
  ...
  if (bench_1)
  {
    ...
    PrintPlan(*bustub,
              "UPDATE terriers SET token = token + 1 WHERE terrier = 0");
    PrintPlan(*bustub,
              "UPDATE terriers SET token = token - 1 WHERE terrier = 0");
  ...
  {
    auto schema =
        "CREATE TABLE terriers(terrier int PRIMARY KEY, token int, network "
        "int);";
    ...
    PrintPlan(*bustub,
              "UPDATE terriers SET token = token - 1 WHERE terrier = 0");
    PrintPlan(*bustub,
              "UPDATE terriers SET network = 1, token = token + 1000 WHERE "
              "terrier = 0");
    PrintPlan(*bustub,
              "UPDATE terriers SET network = 1, token = token - 1000 WHERE "
              "terrier = 0");
  ...
  for (size_t thread_id = 0; thread_id < bustub_thread_cnt; thread_id++)
  {
    if (bench_1)
    {
      threads.emplace_back(
          [thread_id, bustub_terrier_num, duration_ms, &bustub,
           &total_metrics]()
          {
            Bench1TaskTransfer(thread_id, bustub_terrier_num, duration_ms,
    ...
    if (bench_2)
    {
      if (thread_id % 2 == 0)
      {
        threads.emplace_back(
            [thread_id, bustub_terrier_num, duration_ms, &bustub,
             &total_metrics, &token_adjustment]()
            {
              Bench2TaskJoin(thread_id, bustub_terrier_num, duration_ms,
                             bustub::IsolationLevel::SERIALIZABLE, bustub.get(),
                             total_metrics, token_adjustment);
            });
      }
      else
      {
        threads.emplace_back(
            [thread_id, bustub_terrier_num, duration_ms, &bustub,
             &total_metrics, &token_adjustment]()
            {
              Bench2TaskTransfer(thread_id, bustub_terrier_num, duration_ms,
                                 bustub::IsolationLevel::SERIALIZABLE,
  ...
  threads.emplace_back(
      [duration_ms, &db_size, &bustub]()
      { TaskComputeDbSize(duration_ms, db_size, bustub.get(), "terriers"); });
...

  {
    ...
    if (cnt != expected)
    {
      fmt::println(stderr, "inconsistent total tokens: expected {}, found {}",
                   expected, cnt);
      exit(1);
    }
  }

  {
    ...
    fmt::println(
        stderr,
        "--- the following data might be manually inspected by TAs ---");
  ...
  if (total_metrics.committed_transfer_txn_cnt_ <= commit_threshold)
  {
    fmt::println(
        stderr,
        "too many txn are aborted: {} txn committed, expect more than {}",
