...
auto main(int argc, char** argv) -> int
{
  ...
  bustub::BPlusTree<bustub::GenericKey<8>, bustub::RID,
                    bustub::GenericComparator<8>>
      index("foo_pk", page_id, bpm.get(), comparator);
  ...
  for (size_t thread_id = 0; thread_id < BUSTUB_READ_THREAD; thread_id++)
  {
    threads.emplace_back(std::thread(
        ...
        {
          BTreeMetrics metrics(fmt::format("read  {:>2}", thread_id),
                               duration_ms);
          metrics.Begin();
  ...
  for (size_t thread_id = 0; thread_id < BUSTUB_WRITE_THREAD; thread_id++)
  {
    threads.emplace_back(std::thread(
        [thread_id, &index, duration_ms, &total_metrics]
        {
          BTreeMetrics metrics(fmt::format("write {:>2}", thread_id),
                               duration_ms);
          metrics.Begin();
