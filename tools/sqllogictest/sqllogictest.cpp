...
auto ResultCompare(const std::string& produced_result,
                   const std::string& expected_result,
                   bustub::SortMode sort_mode, bool dump_diff) -> bool
{
...
auto ProcessExtraOptions(const std::string& sql,
                         bustub::BustubInstance& instance,
                         const std::vector<std::string>& extra_options,
                         bool verbose,
                         std::shared_ptr<bustub::CheckOptions>& check_options)
    -> bool
{
  for (const auto& opt : extra_options)
  {
    if (bustub::StringUtil::StartsWith(opt, "ensure:"))
    {
      ...
      {
        if (bustub::StringUtil::Contains(result.str(), "IndexScan") ||
            bustub::StringUtil::ContainsAfter("OPTIMIZER", result.str(),
                                              "Filter"))
        {
      ...
      else if (opt == "ensure:topn")
      {
        ...
        check_options->check_options_set_.emplace(
            bustub::CheckOption::ENABLE_TOPN_CHECK);
      }
      ...
      else if (opt == "ensure:nlj_init_check")
      {
        ...
        check_options->check_options_set_.emplace(
            bustub::CheckOption::ENABLE_NLJ_CHECK);
      }
      else
      {
        throw bustub::NotImplementedException(
            fmt::format("unsupported extra option: {}", opt));
      }
    }
    else if (bustub::StringUtil::StartsWith(opt, "timing"))
    {
      ...
      for (; iter != args.cend(); iter++)
      {
        ...
        {
          throw bustub::NotImplementedException(
              fmt::format("unsupported arg: {}", *iter));
        }
      }
...
{  // NOLINT
  ...
  program.add_argument("-d", "--diff")
      .help("write diff file")
      .default_value(false)
      .implicit_value(true);
  program.add_argument("--in-memory")
      .help("use in-memory backend")
      .default_value(false)
      .implicit_value(true);
    ...
    switch (record->type_)
    {
      ...
      case bustub::RecordType::STATEMENT:
      {
        const auto& statement =
            dynamic_cast<const bustub::StatementRecord&>(*record);
        if (verbose)
        {
          fmt::print("{}\n", statement.sql_);
        ...
        try
        {
          if (!ProcessExtraOptions(statement.sql_, *bustub,
                                   statement.extra_options_, verbose,
                                   check_options))
          {
      ...
      case bustub::RecordType::QUERY:
      {
        ...
        try
        {
          if (!ProcessExtraOptions(query.sql_, *bustub, query.extra_options_,
                                   verbose, check_options))
          {
            fmt::print("failed to process extra options\n");
            return 1;
          }
          ...
          if (!ResultCompare(result.str(), query.expected_result_,
                             query.sort_mode_, diff))
          {
            if (diff)
            {
              fmt::print(
                  "wrong result (with sort_mode={}) dumped to result.log and "
                  "expected.log\n",
                  query.sort_mode_);
            }
            else
            {
              fmt::print(
                  "wrong result (with sort_mode={}), use `-d` to store your "
                  "result and expected result in a file\n",
                  query.sort_mode_);
            }
            return 1;
          }
