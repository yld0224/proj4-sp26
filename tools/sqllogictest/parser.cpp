    ...
    -> std::vector<std::string>
{
  ...
  while (last_pos < length + 1)
  {
    ...
    {
      tokens.emplace_back(
          std::string(str.cbegin() + last_pos, str.cbegin() + pos));
    }
...
auto ParseInner(const std::string& filename, const std::string& script)
    -> std::vector<std::unique_ptr<Record>>
{
  auto lines =
      StringUtil::Split(StringUtil::Replace(script, "\r\n", "\n"), '\n');
  ...
  while (line_iter != lines.cend())
  {
    ...
    auto loc = Location{
        filename, static_cast<size_t>(line_iter - lines.cbegin()) + 1, nullptr};
    ...
    if (tokens[0] == "sleep")
    {
      if (tokens.size() == 2)
      {
        records.emplace_back(
            std::make_unique<SleepRecord>(loc, std::stoi(tokens[1])));
      }
      else
      {
        throw bustub::Exception("unexpected number of args");
    ...
    if (tokens[0] == "statement")
    {
        ...
        if (iter->length() > 0)
        {
          if ((*iter)[0] == '+')
          {
            extra_options.emplace_back(
                std::string(iter->cbegin() + 1, iter->cend()));
          }
        }
      }
      ...
      records.emplace_back(std::make_unique<StatementRecord>(
          loc, error, std::move(sql), std::move(extra_options)));
      if (line_iter == lines.cend())
      {
        break;
      }
    ...
    if (tokens[0] == "query")
    {
      ...
      StringUtil::RTrim(&sql);
      records.emplace_back(std::make_unique<QueryRecord>(
          loc, sort_mode, std::move(sql), std::move(expected_result),
...
auto SQLLogicTestParser::Parse(const std::string& script)
    -> std::vector<std::unique_ptr<Record>>
{
