...
// BusTub SQLLogicTest parser is a C++ replication of
// risinglightdb/sqllogictest-rs. SQLLogicTest is SQLite's testing framework.
...
{
  public:
  explicit Record(RecordType type, Location loc)
      : type_(type), loc_(std::move(loc))
  {
  }
...
class IncludeRecord : public Record
{
  ...
  explicit IncludeRecord(Location loc, std::string filename)
      : Record{RecordType::INCLUDE, std::move(loc)},
        filename_(std::move(filename)){};

  auto ToString() const -> std::string override
  {
    return fmt::format("Include {{ filename={} }}", filename_);
  }
...
{
  public:
  explicit QueryRecord(Location loc, SortMode sort_mode, std::string sql,
  ...
  auto ToString() const -> std::string override
  {
    return fmt::format(
        "Query {{\n  sort_mode={},\n  sql={},\n  expected_result=\n{}}}",
        sort_mode_, sql_, expected_result_);
  }
...
{
  public:
  explicit SleepRecord(Location loc, size_t seconds)
      : Record{RecordType::SLEEP, std::move(loc)}, seconds_(seconds){};

  auto ToString() const -> std::string override
  {
    return fmt::format("Sleep {{ seconds={} }}", seconds_);
  }
...
{
  public:
  explicit HaltRecord(Location loc)
      : Record{RecordType::HALT, std::move(loc)} {};
...
{
  public:
  static auto Parse(const std::string& script)
  ...
  auto format(const bustub::Location& c, FormatContext& ctx) const
  {
    if (c.parent_ != nullptr)
    {
      return formatter<string_view>::format(
          fmt::format("{}:{} in {}", c.file_, c.line_, c.parent_), ctx);
    }
    return formatter<string_view>::format(
        fmt::format("{}:{}", c.file_, c.line_), ctx);
  }
};

template <>
struct fmt::formatter<std::shared_ptr<bustub::Location>>
    : formatter<string_view>
{
  template <typename FormatContext>
  auto format(const std::shared_ptr<bustub::Location>& c,
...
struct fmt::formatter<
    T, std::enable_if_t<std::is_base_of<bustub::Record, T>::value, char>>
...
struct fmt::formatter<
    std::unique_ptr<T>,
    std::enable_if_t<std::is_base_of<bustub::Record, T>::value, char>>
    : fmt::formatter<std::string>
{
  template <typename FormatCtx>
