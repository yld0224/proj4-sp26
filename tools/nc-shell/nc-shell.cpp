...
auto GetWidthOfUtf8(const void* beg, const void* end, size_t* width) -> int
{
  ...
  utf8proc_ssize_t size =
      static_cast<const char*>(end) - static_cast<const char*>(beg);
...
void Serve(int socket, bustub::BustubInstance* bustub,
           bool use_serializable_iso_lvl)
{
  ...
  auto txn = bustub->txn_manager_->Begin(
      use_serializable_iso_lvl ? bustub::IsolationLevel::SERIALIZABLE
  ...
  while (true)
  {
    std::string txn_status = fmt::format(
        "txn_id={} status={} read_ts={} iso_lvl={}\n",
    ...
    while (true)
    {
      ...
      if (valread <= 0)
      {
        fmt::println(stderr, "txn{}: disconnected, status={}",
            ...
            txn->GetTransactionState() == bustub::TransactionState::TAINTED)
        {
          ...
          fmt::println(stderr, "txn{}: aborted due to disconnected",
                       txn->GetTransactionIdHumanReadable());
        }
        close(socket);
        return;
      ...
      if (bustub::StringUtil::EndsWith(query, ";") ||
          bustub::StringUtil::StartsWith(query, "\\"))
      {
        break;
      }
      query += "\n";
    ...
    std::cout << "txn" << txn->GetTransactionIdHumanReadable() << ": " << query
              << std::endl
              << std::flush;
    try
    {
...
void Cli(bustub::BustubInstance* bustub)
{
  ...
  std::cout << "In this terminal, all statements are running in separate "
               "transactions. To create transactions, use "
...
auto main(int argc, char** argv) -> int
{
  ...
  while (true)
  {
    int new_socket = accept(
        server_fd, reinterpret_cast<struct sockaddr*>(&address), &addrlen);
    if (new_socket < 0)
    {
      perror("accept");
      exit(EXIT_FAILURE);
    }
    std::thread thread(Serve, new_socket, bustub.get(),
                       use_serializable_iso_lvl);
    thread.detach();
  }
