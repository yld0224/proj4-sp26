...
TEST(TxnScanTest, DISABLED_TupleReconstructTest)
{  // NOLINT
  ...
  }
  {
    ...
    auto undo_log_1 =
        UndoLog{false,
                {true, true, true},
                Tuple{{Int(1), Double(2), Bool(false)}, schema.get()}};
    {
      fmt::println(stderr, "B2: verify 1st record");
      auto tuple =
          ReconstructTuple(schema.get(), base_tuple, base_meta, {undo_log_1});
      ASSERT_TRUE(tuple.has_value());
  ...
  {
    fmt::println(stderr,
                 "C: reconstruct 4 records, one of them is empty, one of them "
                 "is full change");
    auto base_tuple = Tuple{{Int(0), Double(1), BoolNull()}, schema.get()};
    auto base_meta = TupleMeta{2333, false};
    auto undo_log_1_schema = ParseCreateStatement("");
    auto undo_log_1 = UndoLog{
        false, {false, false, false}, Tuple{{}, undo_log_1_schema.get()}};
    auto undo_log_2_schema = ParseCreateStatement("b double");
    auto undo_log_2 = UndoLog{false,
                              {false, true, false},
                              Tuple{{Double(2)}, undo_log_2_schema.get()}};
    auto undo_log_3_schema = ParseCreateStatement("a integer,c boolean");
    auto undo_log_3 =
        UndoLog{false,
                {true, false, true},
                Tuple{{Int(3), Bool(false)}, undo_log_3_schema.get()}};
    auto undo_log_4 =
        UndoLog{false,
                {true, true, true},
                Tuple{{Int(4), Double(4), Bool(true)}, schema.get()}};
    {
    ...
    }
    {
      ...
      auto tuple = ReconstructTuple(schema.get(), base_tuple, base_meta,
                                    {undo_log_1, undo_log_2, undo_log_3});
      ASSERT_TRUE(tuple.has_value());
    ...
    }
    {
      ...
      auto tuple =
          ReconstructTuple(schema.get(), base_tuple, base_meta,
                           {undo_log_1, undo_log_2, undo_log_3, undo_log_4});
      ASSERT_TRUE(tuple.has_value());
  ...
  }
  {
    ...
    auto delete_log =
        UndoLog{true, {false, false, false}, Tuple{{}, delete_schema.get()}};
    auto undo_log_1 =
        UndoLog{false,
                {true, true, true},
                Tuple{{Int(1), Double(1), Bool(false)}, schema.get()}};
    auto undo_log_2 =
        UndoLog{false,
                {true, true, true},
                Tuple{{IntNull(), DoubleNull(), BoolNull()}, schema.get()}};
    {
      ...
      auto tuple =
          ReconstructTuple(schema.get(), base_tuple, base_meta, {delete_log});
      ASSERT_FALSE(tuple.has_value());
    }
    {
      fmt::println(stderr, "D2: verify 2nd record");
      auto tuple = ReconstructTuple(schema.get(), base_tuple, base_meta,
                                    {delete_log, undo_log_1});
      ASSERT_TRUE(tuple.has_value());
    ...
    }
    {
      ...
      auto tuple = ReconstructTuple(schema.get(), base_tuple, base_meta,
                                    {delete_log, undo_log_1, delete_log});
      ASSERT_FALSE(tuple.has_value());
    }
    {
      ...
      auto tuple =
          ReconstructTuple(schema.get(), base_tuple, base_meta,
                           {delete_log, undo_log_1, delete_log, undo_log_2});
      ASSERT_TRUE(tuple.has_value());
...
TEST(TxnScanTest, DISABLED_ScanTest)
{  // NOLINT
  ...
  auto table_info =
      bustub->catalog_->CreateTable(nullptr, "maintable", *schema);

  // record1: txn4 (val=1) -> ts=1 in txn4 (val=2/prev_log_1)
  // record2: ts=3 (val=3) -> ts=2 in txn_store_3 (delete/prev_log_2) -> ts=1 in
  // txn_store_2 (val=4/prev_log_3) record3: ts=4 (delete) -> ts=3 in
  // txn_store_4 (val=5/prev_log_4) record4: txn3 (delete) -> ts=2 in txn3
  // (val=6/prev_log_5) -> ts=1 in txn_store_2 (val=7/prev_log_6)
...

  {
    ...
    prev_log_3 = txn_store_2->AppendUndoLog(
        UndoLog{false,
                ...
                {}});
    prev_log_6 =
        txn_store_2->AppendUndoLog(UndoLog{false,
                                           {true, false, false},
                                           Tuple{{Int(7)}, modify_schema.get()},
                                           1,
                                           {}});
...

  {
    ...
    prev_log_2 =
        txn_store_3->AppendUndoLog(UndoLog{true,
                                           {false, false, false},
                                           Tuple{{}, empty_schema.get()},
                                           2,
                                           prev_log_3});
  ...
  prev_log_5 = txn3->AppendUndoLog(
      UndoLog{false,
              {true, true, true},
              Tuple{{Int(6), DoubleNull(), BoolNull()}, schema.get()},
              2,
              prev_log_6});

  {
    ...
    prev_log_4 = txn_store_4->AppendUndoLog(
        UndoLog{false,
                {true, true, true},
                Tuple{{Int(5), Double(3), Bool(false)}, schema.get()},
                3,
                {}});
  ...
  prev_log_1 = txn4->AppendUndoLog(UndoLog{false,
                                           {true, false, false},
                                           Tuple{{Int(2)}, modify_schema.get()},
                                           1,
                                           {}});

  auto rid1 = *table_info->table_->InsertTuple(
      TupleMeta{txn4->GetTransactionTempTs(), false},
      Tuple{{Int(1), DoubleNull(), BoolNull()}, schema.get()});
  bustub->txn_manager_->UpdateVersionLink(rid1, VersionUndoLink{prev_log_1},
                                          nullptr);
  auto rid2 = *table_info->table_->InsertTuple(
      TupleMeta{txn3->GetReadTs(), false},
      Tuple{{Int(3), DoubleNull(), BoolNull()}, schema.get()});
  bustub->txn_manager_->UpdateVersionLink(rid2, VersionUndoLink{prev_log_2},
                                          nullptr);
  auto rid3 = *table_info->table_->InsertTuple(
      TupleMeta{txn4->GetReadTs(), true},
      Tuple{{IntNull(), DoubleNull(), BoolNull()}, schema.get()});
  bustub->txn_manager_->UpdateVersionLink(rid3, VersionUndoLink{prev_log_4},
                                          nullptr);
  auto rid4 = *table_info->table_->InsertTuple(
      TupleMeta{txn3->GetTransactionTempTs(), true},
      Tuple{{IntNull(), DoubleNull(), BoolNull()}, schema.get()});
  bustub->txn_manager_->UpdateVersionLink(rid4, VersionUndoLink{prev_log_5},
                                          nullptr);

  TxnMgrDbg("before verify scan", bustub->txn_manager_.get(), table_info,
  ...
  // hidden tests... this is the only hidden test case among task 1, 2, 3. We
  // recommend you to implement `TxnMgrDbg` function, draw the version chain
  // out, and think of what should be read by each txn.

  // though we don't have null and double / bool types in task 3 and onwards, we
  // will test them in this test case. you should think about types other than
  // integer, and think of the case where the user updates / inserts a column of
  // null.
  ...
  // <- you will need to fill in the answer fmt::println(stderr, "D: Verify
  // txn3"); WithTxn(txn3, QueryHideResult(*bustub, _var, _txn, query,
  // IntResult{})); // <- you will need to fill in the answer
  // fmt::println(stderr, "E: Verify txn4");
  // WithTxn(txn4, QueryHideResult(*bustub, _var, _txn, query, IntResult{})); //
  // <- you will need to fill in the answer fmt::println(stderr, "F: Verify
  // txn5"); WithTxn(txn5, QueryHideResult(*bustub, _var, _txn, query,
  // IntResult{})); // <- you will need to fill in the answer
}
