...
TEST(TxnIndexTest, DISABLED_IndexConcurrentInsertTest)
{  // NOLINT
  ...
  for (int n = 0; n < trials; n++)
  {
    ...
    fmt::println(stderr, "trial {}: running with {} threads with {} rows",
                 n + 1, thread_cnt, number_cnt);
    global_disable_execution_exception_print.store(true);
    for (int thread = 0; thread < thread_cnt; thread++)
    {
      insert_threads.emplace_back(
          [&bustub, thread, generate_sql, &result_mutex, &operation_result]()
          {
            NoopWriter writer;
            ...
            for (int i = 0; i < number_cnt; i++)
            {
              ...
              if (bustub->ExecuteSqlTxn(sql, writer, txn))
              {
                ...
                BUSTUB_ENSURE(bustub->txn_manager_->Commit(txn),
                              "cannot commit??");
    ...
    WithTxn(query_txn,
            QueryShowResult(*bustub, _var, _txn, "SELECT * FROM maintable",
                            expected_rows));
    auto entry =
        TableHeapEntry(*bustub, bustub->catalog_->GetTable("maintable"));
...
TEST(TxnIndexTest, DISABLED_IndexConcurrentUpdateTest)
{  // NOLINT
  const auto generate_sql = [](int thread_id, int n) -> std::string
  {
    return fmt::format("UPDATE maintable SET b = b + {} WHERE a = {}",
                       (1 << thread_id), n);
  };
  ...
  for (int n = 0; n < trials; n++)
  {
    ...
    TableHeapEntryNoMoreThan(*bustub, bustub->catalog_->GetTable("maintable"),
                             number_cnt);
    ...
    fmt::println(
        stderr,
        "trial {}: running with {} threads with {} rows, add_delete_insert={}",
        n + 1, thread_cnt, number_cnt, add_delete_insert);
    global_disable_execution_exception_print.store(true);
    for (int thread = 0; thread < thread_cnt; thread++)
    {
      update_threads.emplace_back(
          [add_delete_insert, &bustub, thread, generate_sql,
           generate_select_sql, generate_delete_sql, generate_txn_insert_sql,
           &result_mutex, &operation_result]()
          {
            NoopWriter writer;
            std::vector<bool> result;
            result.reserve(number_cnt);
            for (int i = 0; i < number_cnt; i++)
            {
              ...
              if (add_delete_insert)
              {
                ...
                BUSTUB_ENSURE(bustub->ExecuteSqlTxn(generate_select_sql(i),
                                                    data_writer, txn),
                              "cannot retrieve data");
                BUSTUB_ENSURE(data_writer.values_.size() == 1,
                              "more than 1 row fetched??");
                const auto b_val = std::stoi(data_writer.values_[0][0]);
                BUSTUB_ENSURE(bustub->ExecuteSqlTxn(generate_delete_sql(i),
                                                    data_writer, txn),
                              "cannot delete data");
                BUSTUB_ENSURE(
                    bustub->ExecuteSqlTxn(generate_txn_insert_sql(b_val, i),
                                          data_writer, txn),
                    "cannot insert data");
              }
              BUSTUB_ENSURE(bustub->txn_manager_->Commit(txn),
...
TEST(TxnIndexTest, DISABLED_IndexConcurrentUpdateAbortTest)
{  // NOLINT
