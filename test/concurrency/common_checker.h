...
/// @param v1_vec Elements to be insert into `t1:v1`, the size of which can
/// either be 1, or same as @v2_vec
/// @param v2_vec Elements to be insert into `t1:v2`, must not be empty
/// @param flag When set to `true`, the size of @v1_vec should be exactly one,
/// the default value is `false` Usage: Insert(txn, instance, {1, 2, 3}, {4, 5,
/// 6}, flag = false); This overloaded function will then bound each pair from
/// @v1_vec & @v2_vec as a insert value If flag is set to `true`, v1_vec should
/// contain exactly one element, The insert pair will be {v1_vec[0], v2_vec[0]},
/// {v1_vec[0], v2_vec[1]} ...
auto Insert(Transaction* txn, BustubInstance& instance,
            const std::vector<int>& v1_vec, const std::vector<int>& v2_vec,
            bool flag = false) -> void
{
  ...
  if (!flag)
  {
    ...
    {
      fmt::print(stderr,
                 "Vectors @v1_vec and @v2_vec must have the same size\n");
      return;
    }
    if (v1_vec.empty())
    {
  ...
  else
  {
    if (v1_vec.size() != 1)
    {
      fmt::print(
          stderr,
          "Size of @v1_vec should be exactly 1 if flag is set to `true`\n");
      return;
    }
    if (v2_vec.empty())
    {
  ...
  fmt::print(stderr,
             "insert data with @v1_vec = {} and @v2_vec = {} in txn {} {}\n",
             v1_vec, v2_vec, txn->GetTransactionId(), txn->GetIsolationLevel());
  ...
  {
    fmt::print(stderr,
               "Failed to insert data with @v1_vec = {} and @v2_vec = {} in "
               "txn {} {}\n",
               v1_vec, v2_vec);
  }
...
/// @param d_size The size you expect to be deleted from t1, will be used as
/// sanity check, default value is 3 This overloaded function performs a
/// vectorized self-constructed deletion to better test the implementation You
/// should calculate the number of elements that logically should be deleted,
/// and pass it in as @d_size.
auto Delete(Transaction* txn, BustubInstance& instance,
            const std::vector<int>& d_vec, int d_size = 3) -> void
{
  if (d_vec.empty())
  {
    ...
    -> std::shared_ptr<BustubInstance>
{
  ...
  instance->ExecuteSql(
      "INSERT INTO t1 VALUES (233, 1), (233, 2), (233, 3), (234, 1), (234, 2), "
      "(234, 3)",
      writer);
  return instance;
}
                  ...
                  bool read_before_write) -> Transaction*
{
  ...
  if (read_before_write)
  {
    ...
    {
      fmt::print(stderr,
                 "ERROR: initial read failed, force abort, you may see leak "
                 "sanitizer warning and you can ignore "
                 "it\n");
      return nullptr;
    }
  }
