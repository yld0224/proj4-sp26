...
#include <AvailabilityMacros.h>
    ...
    !defined(MAC_OS_X_VERSION_10_14) ||        \
    MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_14
// C++17 new(size_t, align_val_t) is not backwards-compatible with older
// versions of macOS, so we can't support over-alignment in this case
...
class MOODYCAMEL_MAYBE_ALIGN_TO_CACHELINE ReaderWriterQueue
{
...
#endif
  {
    ...
    largestBlockSize = ceilToPow2(
        size + 1);  // We need a spare slot to fit size elements in the block
    if (largestBlockSize > MAX_BLOCK_SIZE * 2)
    {
      // We need a spare block in case the producer is writing to a different
      // block the consumer is reading from, and wants to enqueue the maximum
      // number of elements. We also need a spare element in each block to avoid
      // the ambiguity between front == tail meaning "empty" and "full". So the
      // effective number of slots that are guaranteed to be usable at any time
      // is the block size - 1 times the number of blocks - 1. Solving for size
      // and applying a ceiling to the division gives us (after simplifying):
  ...
  bool try_dequeue(U&& result) AE_NO_TSAN
  {
    ...
    //     return false Else advance to next block and dequeue the item there

    // Note that we have to use the value of the tail block from before we check
    // if the front block is full or not, in case the front block is empty and
    // then, before we check if the tail block is at the front block or not, the
    // producer fills up the front block *and moves on*, which would make us
    // skip a filled block. Seems unlikely, but was consistently reproducible in
    // practice. In order to avoid overhead in the common case, though, we do a
    // double-checked pattern where we have the fast path if the front block is
    // not empty, then read the tail block, then re-read the front block and
    // check if it's not empty again, then check if the tail block has advanced.
  ...
  // Safe to call from both the producer and consumer threads.
  //
  // NOTE: The actual capacity during usage may be different depending on the
  // consumer.
  //       If the consumer is removing elements concurrently, the producer
  //       cannot add to the block the consumer is removing from until it's
  //       completely empty, except in the case where the producer was writing
  //       to the same block the consumer was reading from the whole time.
...
#endif
  {
    ...
    else
    {
      ...
      {
        // Note that the reason we can't advance to the frontBlock and start
        // adding new entries there is because if we did, then dequeue would
        // stay in that block, eventually reading the new values, instead of
        // advancing to the next full block (whose values were enqueued first
        // and so should be consumed first).

        fence(memory_order_acquire);  // Ensure we get latest writes if we got
                                      // the latest frontBlock
...
private:
  ...
  struct Block
  {
    // Avoid false-sharing by putting highly contended variables on their own
    // cache lines
    weak_atomic<size_t> front;  // (Atomic) Elements are read from here
    size_t
        localTail;  // An uncontended shadow copy of tail, owned by the consumer
    ...
    char cachelineFiller1[MOODYCAMEL_CACHE_LINE_SIZE -
                          ...
                          sizeof(size_t)];  // next isn't very contended, but we
                                            // don't want it on the same cache
...
public:
  ...
  char
      cachelineFiller[MOODYCAMEL_CACHE_LINE_SIZE - sizeof(weak_atomic<Block*>)];
  weak_atomic<Block*>
      tailBlock;  // (Atomic) Elements are enqueued to this block
...
class BlockingReaderWriterQueue
{
  ...
  inline bool wait_dequeue_timed(
      U& result, std::chrono::duration<Rep, Period> const& timeout) AE_NO_TSAN
  {
