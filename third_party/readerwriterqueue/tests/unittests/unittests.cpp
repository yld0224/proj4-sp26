...
class ReaderWriterQueueTests : public TestClass<ReaderWriterQueueTests>
{
  ...
  {
    {
      ...
      for (size_t q_size = 2; q_size < 256; ++q_size)
      {
        ...
        for (size_t i = 0; i < start_cap + 1;
             ++i)  // fill 1 past capacity to resize
          q.enqueue(i);
  ...
  // This is what you have to do to try_enqueue() a movable type, and
  // demonstrates why try_emplace() is useful
  bool try_enqueue_fail_workaround()
  {
    ...
    {
      // Workaround isn't pretty and potentially expensive - use try_emplace()
      // instead
      std::unique_ptr<int> p{new int(123)};
  ...
  {
    {
      ...
      {  // check there's no problem with mismatch between nominal and allocated
         // capacity
        ...
        for (int i = 1; i != 65; ++i) q.wait_enqueue(i);
    ...
    Foo::reset();
    {
      ...
      {
        Foo item;
        for (int i = 0; i != 10; ++i) ASSERT_OR_FAIL(q.try_enqueue(Foo()));
...

    {
      ...
      SimpleThread writer(
          [&]()
          {
            for (int i = 0; i != 1000000; ++i) q.wait_enqueue(i);
          });
...
int main(int argc, char** argv)
{
  ...
  // Disable buffering (so that when run in, e.g., Sublime Text, the output
  ...
  {
    std::printf(
        "Running all unit tests for moodycamel::ReaderWriterQueue.\n(Run %s "
