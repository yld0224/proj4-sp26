          ...
          [&]()
          {
            ...
            for (unsigned long long j = 0; j < 1024ULL * 1024ULL * 32ULL;)
            {
              ...
              if (q.try_dequeue(element))
              {
                if (element != j)
                {
                  log << "  ERROR DETECTED: Expected to read " << j
                      << " but found " << element << std::endl;
                  std::printf(
                      "  ERROR DETECTED: Expected to read %llu but found %llu",
                      j, element);
                }
                ++j;
                canLog = true;
            ...
            if (q.try_dequeue(element))
            {
              log << "  ERROR DETECTED: Expected queue to be empty"
                  << std::endl;
