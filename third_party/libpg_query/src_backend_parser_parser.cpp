...
PGList* raw_parser(const char* str)
{
  ...
  yyscanner =
      scanner_init(str, &yyextra.core_yy_extra, ScanKeywords, NumScanKeywords);
...
std::vector<PGSimplifiedToken> tokenize(const char* str)
{
  ...
  while (true)
  {
    ...
    switch (token)
    {
      case IDENT:
        current_token.type =
            PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_IDENTIFIER;
        break;
      case ICONST:
      case FCONST:
        current_token.type =
            PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_NUMERIC_CONSTANT;
        break;
      case SCONST:
      case BCONST:
      case XCONST:
        current_token.type =
            PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_STRING_CONSTANT;
        break;
      case Op:
      case PARAM:
      case COLON_EQUALS:
      ...
      case NOT_EQUALS:
        current_token.type =
            PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_OPERATOR;
        break;
      default:
        if (token >= 255)
        {
          ...
          current_token.type =
              PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_KEYWORD;
        }
        else
        {
...
int base_yylex(YYSTYPE* lvalp, YYLTYPE* llocp, core_yyscan_t yyscanner)
{
  ...
  yyextra->lookahead_end =
      yyextra->core_yy_extra.scanbuf + *llocp + cur_token_length;
