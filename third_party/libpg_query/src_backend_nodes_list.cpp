 ...
 */
static void check_list_invariants(const PGList* list)
{
  if (list == NIL)
    return;
  ...
  Assert(list->type == T_PGList || list->type == T_PGIntList ||
 ...
 */
static PGList* new_list(PGNodeTag type)
{
  PGList* new_list;
  PGListCell* new_head;
 ...
 * sure to fill it in
 */
static void new_head_cell(PGList* list)
{
  PGListCell* new_head;
 ...
 * sure to fill it in
 */
static void new_tail_cell(PGList* list)
{
  PGListCell* new_tail;
 ...
 */
static void list_free_private(PGList* list, bool deep)
{
  PGListCell* cell;
 ...
 */
void list_free(PGList* list) { list_free_private(list, false); }
...
PGList* list_copy_tail(const PGList* oldlist, int nskip)
{
  ...
  while (nskip-- > 0) oldlist_cur = oldlist_cur->next;
