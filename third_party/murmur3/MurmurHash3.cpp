...
static FORCE_INLINE uint64_t getblock(const uint64_t* p, int i) { return p[i]; }
...
int32_t MurmurHash3_x64_128(const void* key, const int len, const uint32_t seed)
{
  ...
  switch (len & 15)
  {
    ...
    case 9:
      k2 ^= uint64_t(tail[8]) << 0;  // fallthrough
      k2 *= c2;
      k2 = ROTL64(k2, 33);
      k2 *= c1;
      h2 ^= k2;  // fallthrough
    ...
    case 1:
      k1 ^= uint64_t(tail[0]) << 0;  // fallthrough
      k1 *= c1;
      k1 = ROTL64(k1, 31);
      k1 *= c2;
      h1 ^= k1;  // fallthrough
  ...
  // Shift so that we use the higher order bits in case we want to use the lower
  // order ones later Also use the h1 higher order bits because it provided much
  // better performance in voter, consistent too
